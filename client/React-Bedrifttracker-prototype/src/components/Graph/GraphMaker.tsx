...
export const GraphMaker = () => {
    ...
    return (
        ...
        {errorState ? <p className={style.errorMessage}>{data.result.message}</p> : dataset === null ? <><p>Waiting for Data</p><GraphSkeleton/></> : <BarChart dataset={dataset} xAxis={[{scaleType: "band", dataKey: 'year'}]} series={[{dataKey: 'companyData', label: currentCompany ? currentCompany : ""}, {dataKey: 'comparisonData', label: "Average Values in Database"}]} height={600} width={600}/>}
        </>
    )
}