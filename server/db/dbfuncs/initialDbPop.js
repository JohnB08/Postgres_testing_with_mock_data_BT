 ...
 * @returns
 */
const verifyProffData = (data) => {
    return (typeof data === "object" &&
        Array.isArray(data.annualAccounts) &&
        data.annualAccounts.length > 0 &&
        data.companyId);
};
...
const seedDbWithDataFromExcel = async () => {
    ...
    try {
        for (let company of excelData) {
            ...
            const companyId = insertIntoCompanyName.rows[0].bedrift_id;
 ...
 * @returns
 */
const cleanProffData = (proffData) => {
    let cleanedData = [];
...
const combineReferenceAndProff = (proffData, referenceData) => {
    const newData = [];
    for (let data of proffData) {
...
const insertInitialProffData = async (proffData) => {
    const errorArray = [];
